        ...
        <div class="col-lg-4 col-md-4 col-sm-6 col-6 custom-col mb-30">
            <article class="product__card">
                ...
                <div class="product__card--content">
                    ...
                    <div class="product__card--footer add_to_cart"  data-color-cond="0" data-product_slug="${product.slug}" data-size-cond="0" data-size-id="${product.size}" data-color-id="${product.color}">
 ...
 const product_list_html= (item)=>{
    ...
    <div class="product__card product__list d-flex align-items-center">
        ...
        <div class="product__card--content product__list--content">
            ...
            <a class="product__card--btn primary__btn add_to_cart"  href="javascript:void(0)" data-product_slug="${product.slug}" data-size-cond="0" data-size-id="${product.size}" data-color-cond="0" data-color-id="${product.color}">+ Add to cart</a>
        </div>
    </div>
</div>`;
...
const radio_btn_event = (btn)=>{
    let type = btn.getAttribute("data-type");
        let value = btn.getAttribute("data-value");

        if (advance_filter.hasOwnProperty(type)) {

                if (advance_filter[type]==value) {
                    delete advance_filter[type];
                    if(btn.classList.contains('active_filter_btn')){
                        btn.classList.remove('active_filter_btn')
                    }
                }else{

                    advance_filter[type]=value;
                    let active_btn = btn.closest("ul").querySelector(".active_filter_btn");
                    if (active_btn) {
                        active_btn.classList.remove('active_filter_btn');
...
const filter_column_value_change = (obj)=>{
    ...
    for(let i=0;i<size.length;i++){
        for (let x of size_list_parent){
            let li = document.createElement("li");
                li.classList.add("widget__tagcloud--list");
                li.innerHTML = `<a class="widget__tagcloud--link radio_btn" data-value="${size[i]}" data-type="size"> ${size[i]}
                </a>`;

                x.append(li);
                li.addEventListener("click",()=>{
                    radio_btn_event(li.querySelector('a'));
                });
        }
    }
    ...
    for(let c of color_list){
     ...
     for(let i=0;i<color.length;i++){
         for (let x of color_list_parent){
             let li = document.createElement("li");
                 li.classList.add("widget__tagcloud--list");
                 li.innerHTML = `<a class="widget__tagcloud--link radio_btn" data-value="${color[i]}" data-type="color"> ${color[i]}
                 </a>`;
